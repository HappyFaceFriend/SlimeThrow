...
public class SlimeBehaviour : StateMachineBase
{
    ...
    public void OnHittedByPlayer(PlayerBehaviour player, float damage)
    {
        ...
        if (GlobalRefs.UpgradeManager.GetCount("냉동고") >= 1 && _hpSystem.CurrentHp > 0)
            ApplyBuff(new SlimeBuffs.Freeze(1f, this));
    }
    public void OnHittedByBullet(Vector3 landPosition, float damage)
    {
        ...
        if (GetComponent<KingBehaviour>() == null && _hpSystem.CurrentHp < (_hpSystem.MaxHp.Value / 2) && GlobalRefs.UpgradeManager.GetCount("파이어 슬래이어") != 0 && GlobalRefs.Turret._bulletBuilder._slimeName == "Fire Slime")
            OnGetHitted(impactPosition, 999f, true);
        else
            OnGetHitted(impactPosition, damage, false);
        if (GlobalRefs.UpgradeManager.GetCount("파이어 캐논") != 0 && _hpSystem.CurrentHp != 0)
            ApplyBuff(new SlimeBuffs.Burn(4f, 2, 0.8f));
        if (GetComponent<KingBehaviour>() == null && GlobalRefs.UpgradeManager.GetCount("빙결탄") != 0 && _hpSystem.CurrentHp != 0)
